...
\textcolor{red}{TODO:
* Illustrate general concept of intersect and normal.}

\begin{itemize}
\item \lstinline{intersect}: Intersect a ray with the shape.
\item \lstinline{normal}: Provide a normal vector for a $(x_q, y_q)$ 
      position query.
\end{itemize}
...
\paragraph{normal}
The normal operation provides a unit normal vector for a given position
$(x_q, y_q)$ on the shape. The normal vector is expressed in the surface
LCS. The normal vector is a unit vector and has a positive $z$ component.
...
\paragraph{normal}
The plane normal vector is constant. It is $\overrightarrow{N} = (0, 0, 1)$.
...
the query position $(x_q, y_q)$, and with positive $z$ \gls{LCS} coordinate, is
computed as follows.
